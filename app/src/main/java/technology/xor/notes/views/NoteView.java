...
import technology.xor.notes.support.NoteRecyclerViewAdapter;
...
public class NoteView extends Fragment {
    ...
    private void ShowCards() {
        ...
        mAdapter = new NoteRecyclerViewAdapter(getActivity(), cNotes);
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case 2000:
                CipherNotes cipherNotes = cNotes.get(mAdapter.getPosition());
                CipherDatabaseHelper cipherDatabaseHelper = new CipherDatabaseHelper(getActivity());
                cipherDatabaseHelper.DeleteNote(cipherNotes.GetId());
                cipherDatabaseHelper.close();
                cNotes.remove(mAdapter.getPosition());
                mAdapter.notifyDataSetChanged();
                return true;
            case 2001:
                CipherNotes cipherNotes1 = cNotes.get(mAdapter.getPosition());
                System.out.println("Position: " + mAdapter.getPosition());
                Intent intent = new Intent(getActivity().getBaseContext(), NewNote.class);
                Bundle args = new Bundle();
                args.putInt("note_id", cipherNotes1.GetId());
